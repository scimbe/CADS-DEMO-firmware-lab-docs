---
layout: default
title: Your first session (JavaScript)
lang: en
permalink: /en/javascript/first-session/
---
...
Tutor. Click it and the side bar shows the course tree: **JavaScript – Foundations**, its eight modules,
...
The first step is about the window rather than about JavaScript, on purpose. Everything after it assumes
...
- **The first `node` command says it cannot find the file.** Your terminal is one folder above the
  exercises. `cd ~/workspace/javascript-foundations` fixes it. See
...
- **A test fails.** That is the normal state of an exercise. The failed assertion and what it
  expected are the material the step wants you to read; see
...
- [Running commands and tasks]({{ '/en/javascript/working-the-window/' | relative_url }})
- [What the course covers]({{ '/en/javascript/the-course/' | relative_url }})
